...
from lib.backbones import vit
from lib.backbones.vitup import VITUp
...
class CenterNet3D(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', num_class=3, downsample=4, flag='training', model_type='centernet3d', input_type='rgb'):
        ...
        if 'dla' in backbone:
            ...
            fc_channels = channels[self.first_level]
        elif 'vit' in backbone:
            self.backbone = getattr(vit, backbone)(pretrained=True, img_size=(384,1280))
            embed_dim = self.backbone.embed_dim  # channels list for feature maps generated by backbone
            self.neck = VITUp(embed_dim)  # feature fusion [such as DLAup, VITUP, FPN]
            fc_channels = self.neck.out_channel
        ...
        for head in self.heads.keys():
            ...
            fc = nn.Sequential(
                nn.Conv2d(fc_channels, 256, kernel_size=3, padding=1, bias=True),
